...
--
-- Host: localhost
-- Generation Time: Mar 26, 2021 at 12:45 AM
...
INSERT INTO `shop_orders` (`order_id`, `order_placed`, `order_creator`, `order_total`, `order_notes`) VALUES
...
INSERT INTO `shop_products` (`id`, `parentid`, `product_name`, `product_sku`, `product_desc`, `product_image`, `product_price`, `product_stock`) VALUES
(1, 1, 'Margaret', '', 'Agapanthus Margaret', '', '2.00', 93),
(2, 1, 'Navy Blue', '', '-', '', '15.00', 1),
(3, 1, 'Peter Pan', '', '-', '', '15.00', 4),
(4, 3, '1L Compost', '', '-', '', '3.00', 95);
...
-- Events
--
CREATE DEFINER=`root`@`localhost` EVENT `weekly` ON SCHEDULE EVERY 7 DAY STARTS '2021-03-21 00:00:00' ON COMPLETION NOT PRESERVE ENABLE DO UPDATE todo SET completed = 0 WHERE weekly = 1$$

CREATE DEFINER=`root`@`localhost` EVENT `daily` ON SCHEDULE EVERY 1 DAY STARTS '2021-03-25 09:00:00' ON COMPLETION NOT PRESERVE ENABLE DO UPDATE todo SET completed = 0 WHERE daily = 1$$
